...
To simplify building and deploying across local or K8s environments, we use a single unified deployment manager: `scripts/build-deploy-flutter.sh`. By default, it uses Docker, so you do not even need Flutter installed!
...
Available environments defined in `flutter_config.env`: `local`, `k8s`.
...
**Build Android APK (K8s environment variables):**
```bash
scripts/build-deploy-flutter.sh --env k8s --target apk
```
...
- **LAN / `192.168.x.x`:** If the web build still has `localhost` in `API_BASE_URL` / `HLS_URL`, the client rewrites `localhost` / `127.0.0.1` to the **same host as the page**. Rebuild after pulling so this logic ships.
